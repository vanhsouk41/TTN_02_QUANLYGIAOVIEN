...
namespace QUAN_LY_GIAO_VIEN
{
    ...
    public partial class Data_form : Form
    {
        ...
        {
            Export.ExportFile("DANH SÁCH BÀI BÁO KHOA H?C", dataGridView1);
        }
        // Nh?n ch?n 1 dòng
        ...
        {
            DialogResult res = MessageBox.Show("B?n có mu?n l?u k?t qu??", "MediaM", MessageBoxButtons.OKCancel, MessageBoxIcon.Information);
        ...
        {
            DialogResult res = MessageBox.Show("B?n có mu?n h?y?", "MediaM", MessageBoxButtons.OKCancel, MessageBoxIcon.Information);
            if (res == DialogResult.OK)
            {
                string query = "SELECT * FROM  dbo.LOPHOCPHAN";
                DataSet data = new DataSet();
        ...
        {
            Export.ExportFile("DANH SÁCH L?P H?C PH?N", lop_hp_datagitview);
        }
